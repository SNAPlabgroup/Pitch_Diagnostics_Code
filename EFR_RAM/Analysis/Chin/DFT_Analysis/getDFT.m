function [f,DFT,PLV, floory, pos, neg] = getDFT(tot,collected,window,Fs,Fs0,gain,K_MRS, NF_iters)
%getDFT - Returns f - frequency, and mean DFT with noise floor removed.
%   Assumtions: 
%   -Pull 1/5th of total trials to look at and average 

len = length(tot)/2; 
numtrials = round(collected); % was coll/2 %Number of trials to pull from/polarity, 1/5 of total collected
...
for i = 1:1:len
    ...
    %Neg
    temp2 = tot{ind}(round(window(1)*Fs0):round(window(2)*Fs0))/gain;
