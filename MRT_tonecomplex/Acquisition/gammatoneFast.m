...
%
%   BM = gammatoneFast(X,CFS,FS) passes the vector X through
...
%   as CFS.
%
%   Based on code written by JF Woodruff, University of Surrey in Nov'08
% 
%   Adapted by Hari Bharadwaj (2015-2019)
    ...
    % check inputs
    assert(isvector(x) & isnumeric(x), 'gammatoneFast:invalidX', 'x must be a vector')
    assert(isvector(cfs) & isnumeric(cfs), 'gammatoneFast:invalidCfs', 'cfs must be a vector')
    assert(isscalar(fs), 'gammatoneFast:invalidFs', 'fs must be a scalar')
    assert(islogical(align) & numel(align)==1, 'gammatoneFast:invalidAlign', 'align must be logical')
    ...
    b = 24.7.*(4.37.*cfs./1000+1); % rate of decay or bandwidth
