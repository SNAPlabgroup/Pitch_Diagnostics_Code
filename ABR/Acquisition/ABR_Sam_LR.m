...
clickrate = 21; %Hz
jitwin = 0.04; 
levels = 115; %115 only was original

% Experiment parameters
nconds = numel(levels);
ntrials = 240; % About 20 mins per level
order = repmat(1:nconds, 1, ntrials);
order = order(randperm(nconds*ntrials));
%triglistL = [3, 5, 6, 9, 10, 12]; % extra triggers
%triglistR = [48, 80, 96, 144, 160, 192];
triglistL = [3, 9]; % extra triggers
triglistR = [48, 144];
...
info.measure = 'ABR'; 
info.version = 'v01'; 
stim.stim_type = 'click'; 
stim.totalStimDur = dur; 
stim.clickAmp = stimpe; 
stim.clickDur_ms = clickdur; 
stim.clickRate_hz = clickrate;  
stim.jitwin = jitwin; 
stim.levels = levels; 
stim.ntrials = ntrials; 
stim.triglistL = triglistL; 
stim.triglistR = triglistR;
resp.testdur_s = test_dur; 

data.info = info; 
data.stim = stim; 
data.resp = resp; 

filename = ['./RESULTS/' info.measure '_' visit.subj.ID '_' datetag]; 
save(filename, 'data')
fprintf(1,'\n Saved metadata!\n');