...
cwd = pwd;
cd(datapath)
datafile = {dir(fullfile(cd,['DPOAEswept_', subj, '_*.mat'])).name};
if length(datafile) < 1
    fprintf('No file...Quitting!\n');
elseif size(datafile,2) > 1
...
% title([subj ' | DPOAE | ' condition], 'FontSize', 14)
...
if subj(1) == 'S'
    ...
    checkDIR=dir(sprintf('Calib_Ph1ER-10X_%s*%s*.mat', subj, stim.date(1:11)));
    if isempty(checkDIR)
        fprintf('No such Calibs for subject %s\n',subj);
    elseif size(checkDIR, 1) > 1
        fprintf('Date/Time of Test: %s\n',stim.date); 
        checkDIR =uigetfile(sprintf('Calib_Ph1ER-10X_%s*%s*.mat', subj, stim.date(1:11)));
        load(checkDIR);
    else
        load(checkDIR.name);
    end
...
%                 res.f2_fpl = F2.f;
        ...
        % plot figure again
        figure;
        plot(freq_f2/1000, res.dbEPL_dp, 'linew', 3, 'Color', '#d73027');
        hold on;
...
%% Export:
cd(datapath);
fname = [subj,'_DPOAEswept_',condition];
