...
% errMsg = bekesyBidelmanTask_PTB(subId, expSite)
%   Inputs:
...
whereThresh = ['..' filesep 'Threshold_ForTuning' filesep 'Output']; % path to absolute threshold results
...
dBPerSec = 2;
rms1Level = 102.8; % Enter location-specific calibration level: The headphone output level of a stimulus with an RMS of 1
...
if generateNewSweep % if we want to generate a sweep from scratch...
    ...
    while clipCheck
        ...
        y_scaled = 10^((allRampStartdB(end)-maxLevel)/20).*(y./rms(y)); % scale the noise to the max allowed level
...
try
    ...
    whichSoundDevice = deviceindex; % usually [] is good enough but sometimes need to specify non-default device
    ...
    pamaster = PsychPortAudio('Open',whichSoundDevice,...     % create a handle to the [default soundcard] This should default to the only ASIO card.
        ...
        [8, 9]);                                  % Specific track numbers for [Output; Input] 
                                                                                     
    % HARI: Had to change channel number in 'Open' for it to work
    ...
    while keepGoing
        ...
        if idx(end) > size(y,1)
            ...
            levelRamp = levelRamp(1:length(idx)).*flipud(onsetRamp(1:length(idx))); % adjust which samples of the ramp are used to match the remaining # of samples in the noise, and apply offset ramp
        ...
        if loopCtr > 1
            ...
            bufferdata = [y(idx,1) y(idx,2).*levelRamp.*onsetRamp]';
        end
        ...
        %         if ~respRecorded
        %             if ~isempty(intersect([currLev prevLev], [1 nLevels])) && (diff([prevLev currLev]) ~= 0) % if max or min is hit/diverged from, also collect the data point
        %                 freqIdx = idx(1) + round((idx(end)-idx(1))./2); % compute the index of the center of the current audio segment
        %                 allResp(end+1,:) = [freqSeq(freqIdx) allRampStartdB(currLev)];
        %             end
        %         end
