...
for k = 1:length(chins)
    ...
    for awake = 1       % awake
        ...
        [a_Pre_oae(a_count, :),a_Pre_nf(a_count,:), ...
            a_Pre_f(a_count,:),a_Pre_oaesum(a_count,:), ...
            a_Pre_centerfreq(a_count,:)] = get_data_SF(prefix, subj, 'Baseline', awake);
        
        % Then get 2wks Post data
        [a_Post_oae(a_count, :),a_Post_nf(a_count,:), ...
            a_Post_f(a_count,:),a_Post_oaesum(a_count,:), ...
            a_Post_centerfreq(a_count,:)] = get_data_SF(prefix, subj, [group{1},'_2wksPost'], awake);
    ...
    for awake = 0       %sedated
        ...
        [s_Pre_oae(s_count, :),s_Pre_nf(s_count,:), ...
            s_Pre_f(s_count,:),s_Pre_oaesum(s_count,:), ...
            s_Pre_centerfreq(s_count,:)] = get_data_SF(prefix, subj, 'Baseline', awake);
        
        % Then get 2wks Post data
        [s_Post_oae(s_count, :),s_Post_nf(s_count,:), ...
            s_Post_f(s_count,:),s_Post_oaesum(s_count,:), ...
            s_Post_centerfreq(s_count,:)] = get_data_SF(prefix, subj, [group{1},'_2wksPost'], awake);
...
hold on;
text(.6,-30, sprintf('n = %d', size(a_Pre_f, 1) - sum(isnan(a_Pre_f(:,1)))), 'FontSize', 12)
text(.6,-32.5, sprintf('n = %d', size(a_Post_f, 1) - sum(isnan(a_Post_f(:,1)))), 'Color', post_awake_group, 'FontSize', 12)
title(sprintf('%s | Swept SFOAEs', group{1}),'FontSize',16);
% 
% for i = 1:size(a_Pre_f,1)
%     plot(a_Pre_f(i,:),a_Pre_oae(i,:), 'Color',pre_awake_ind,'linewidth',2.5);
%     plot(a_Pre_centerfreq(i,:),a_Pre_oaesum(i,:),'o','Color',pre_awake_group,'MarkerSize',10);
% end
% 
% for i = 1:size(a_Post_f,1)
%     plot(a_Post_f(i,:),a_Pre_oae(i,:), 'Color',post_awake_ind,'linewidth',2.5);
%     plot(a_Post_centerfreq(i,:),a_Post_oaesum(i,:),'o','Color',post_awake_group,'MarkerSize',10,'LineWidth',2);
% end
% 
% for i = 1:size(s_Pre_f,1)
%     plot(s_Pre_f(i,:),s_Pre_oae(i,:),'Color',pre_sed_ind,'linewidth',2.5);
%     plot(s_Pre_centerfreq(i,:),s_Pre_oaesum(i,:),'*','Color',pre_sed_ind,'MarkerSize',10,'LineWidth',2);
% end
% 
% for i = 1:size(s_Pre_f,1)
%     plot(s_Post_f(i,:),s_Post_oae(i,:),'Color',post_sed_ind,'linewidth',2.5);
%     plot(s_Post_centerfreq(i,:),s_Post_oaesum(i,:),'o','Color',post_sed_group,'MarkerSize',10,'LineWidth',2);
% end

plot(mean(a_Pre_f,1, 'omitNaN'),mean(a_Pre_oae,1, 'omitNaN'),'Color',pre_awake_group,'linewidth',5);
plot(mean(a_Pre_f,1, 'omitNaN'),mean(a_Pre_nf,1, 'omitNaN'),'--', 'Color',pre_awake_group,'linewidth',1.5);
plot(mean(a_Pre_centerfreq,1, 'omitNaN'),mean(a_Pre_oaesum,1, 'omitNaN'),'*','Color',pre_awake_group,'MarkerSize',10,'LineWidth',2);
plot(mean(a_Post_f,1, 'omitNaN'),mean(a_Post_oae,1, 'omitNaN'),'Color',post_awake_group,'linewidth',5);
plot(mean(a_Post_f,1, 'omitNaN'),mean(a_Post_nf,1, 'omitNaN'),'--', 'Color',post_awake_group,'linewidth',1.5);
plot(mean(a_Post_centerfreq,1, 'omitNaN'),mean(a_Post_oaesum,1, 'omitNaN'),'*','Color',post_awake_group,'MarkerSize',10,'LineWidth',2);

plot(mean(s_Pre_f,1, 'omitNaN'),mean(s_Pre_oae,1, 'omitNaN'),'Color',pre_sed_group,'linewidth',5);
plot(mean(s_Pre_f,1, 'omitNaN'),mean(s_Pre_nf,1, 'omitNaN'),'--', 'Color',pre_sed_group,'linewidth',1.5);
plot(mean(s_Pre_centerfreq,1, 'omitNaN'),mean(s_Pre_oaesum,1, 'omitNaN'),'*','Color',pre_sed_group,'MarkerSize',10,'LineWidth',2);
plot(mean(s_Post_f,1, 'omitNaN'),mean(s_Post_oae,1, 'omitNaN'),'Color',post_sed_group,'linewidth',5);
plot(mean(s_Post_f,1, 'omitNaN'),mean(s_Post_nf,1, 'omitNaN'),'--', 'Color',post_sed_group,'linewidth',1.5);
